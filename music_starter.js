...
function draw_one_frame(words, vocal, drum, bass, other, counter) {
  ...
  let colorMap = map(vocal, 0, 100, 255, 0); //didn't end up using

  let bassMap = map(bass, 0, 100, 20, 200);
  let otherMap = map(other, 0, 100, 0, radians(500));
  let vocalMap = map(vocal, 0, 100, 500, 700); // didn't end up using
  ...
  // ChatGPT helped me with some mistakes I made in the code and why it wasn't working
  ...
  if (seconds > 33.5 && seconds < 47) {
    ...
    scale(4);
    noStroke();
    fill(243, 220, 247, 200); // transparent light purple
    ...
    for (let i = 0; i < numEllipses; i++) {
      ...
      // Chat GPT helped with this bit here and the part below
  ...
  // second chorus but turned to light-ish pink
